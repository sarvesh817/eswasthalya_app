...
class EclinicPanel extends MY_Controller {
    ...
    public function appointments() {
		$uid = $this->session->userdata('user_id');
		$data['eclinic_info_edit'] = $this->Mastermodel->master_get('tbl_eclinic','user_id="'.$uid.'"', '*');
		$data['appoint_list'] = $this->Mastermodel->master_get('tbl_book_appointment','', '*');
		$data['specialization_list'] = $this->Mastermodel->master_get('tbl_doctors','', 'specialization');  
    ...
    public function clickToCall(){
		...
		$formdata['app_url'] = 'https://dev.eswasthalya.ssquareconsultants.com/index.html?appid=9e3c2c96c8384758ad69c8155a46a36b&channel='.$dr_id.'&token=';
		$formdata['created_at'] = date("Y-m-d h:i:s");
