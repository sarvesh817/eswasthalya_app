...
class Masterlogin extends CI_Controller {
	 ...
	 public function index()
	 {
	   if($this->session->userdata('user_id') > 0) {
		    ...
		    if($USERTYPE =='SUPERADMIN'){
	 		...
	 		$this->load->view('login/masterlogin');
	    }
	 }
	...
	public function loginNow(){	
		...
		$user_details = $this->mastermodel->master_get('user',"user_type IN('SUPERADMIN') AND email='$email' AND password = '$password'",'user_id, user_type, name, email, status, contact, role_id, subrole_id, password_set_date, last_loggedin, logged_in,availability');
	...
	public function logout() {
		...
		redirect(URL.'superadmin','refresh');
	}
