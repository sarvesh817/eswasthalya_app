...
class Doctor_model extends CI_Model
{
    ...
    public function save_new_doctor($table,$post_data)
    {
		$this->db->where('user_id', $post_data['doctorsid']);  
    ...
    {
        $this->db->where('week_days',date('l'));                
        $this->db->where('user_id',$id);               
        $this->db->select('*');     
...
public function delete_all($table,$profile_id){
    ...
    public function update_column_profile($table,$ciid,$userid){
        if($ciid==1){
            ...
            $update_col_date="signature_date"; 
        }else if($ciid==2){
            $update_col="joining_letter";
            $update_col_date="joining_date"; 
        }else if($ciid==3){
            $update_col="other_document";
            $update_col_date="document_date"; 
        }
        $post_data=array($update_col=>"",$update_col_date=>"");         
        return $this->db->where('user_id',$userid)->update($table,$post_data);   
    }
