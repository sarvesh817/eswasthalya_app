...
ul .icList {background: #FFD;color: #000;list-style-type: none;border-bottom: 1px solid #EEE;width: 300px;margin-left: -25px; padding-left: 5px; cursor: pointer;line-height:2.1em;}
#suggesstion-box{overflow: auto;height: 153px;z-index: 9999 !important;position: absolute;left: 0px;}
    ...
    <section class="content">
        <div class="row">
        	<div class="col-sm-12" style="top: -10px;margin: 0em 0em 0em 0.5em;font-size: 15px;font-weight: 600;">
         		<a href="<?php echo URL.'login';?>">Home</a> &raquo; Schedule Appointment
        	</div>
			<div class="col-sm-12" style="padding : 0px 8px;">
			    <!-- Horizontal Form -->
				<div class="box box-info" id="showafter" style="display: none;">
					<h1 style="text-align: center;">Thank You</h1>
					<h3 style="text-align: center;">Your appointment record has been updated, our customer support team will call you back to recofirm your medical appointment!</h3>
				</div>
				...
				<div class="box-body">	
					<div class="row">
						...
						<div class="col-md-8" style="padding: 10px;">
							...
							<div class="row" id="SerchApplication"> 
								<div class="col-sm-4">
									<label>Insurance Company</label>
									<input type="text" class="form-control" name="ic_name" id="ic_name" value="" autocomplete="off">
									<input type="hidden" class="form-control" id="ic_id" name="ic_id" value="">
									<div id="suggesstion-box" style="display: none"></div>
								</div>
								<div class="col-md-4">
									<label>Application No.</label>
									<input type="text" name="ApplicationNum" class="form-control" id="ApplicationNum">
								</div>
								<div class="col-md-3" style="margin-top: 1.7em;">
									<input type="button" name="SearchCaseData" class="btn btn-primary" id="SearchCaseData" value="Search">
								</div>
							</div>
					...
					<div class="row">
						<div class="col-md-1"></div>
						<div class="col-md-8" id="displayError"></div>
					</div>
			...
			<div class="col-md-8">
				...
				<div class="row">					
					<div class="col-md-6">
						...
						<input type="text" name="area" id="sublocality_level_1" value="" autocomplete="off" class="form-control reqrfld" placeholder="Area" />
						<input type="hidden" name="lat" id="lat" value="" />
						<input type="hidden" name="lng" id="lng" value=""/>						
					</div>					
					<div class="col-md-6">
						<label>Landmarks</label>
						<input type="text" name="landmark" id="sublocality_level_2" value="" autocomplete="off" class="form-control reqrfld" placeholder="Enter Landmark"/>					
					</div>
					<div class="col-md-6">
				...
				<div class="row">
					...
					<div class="col-md-3">
						<label for="cust_type" class="control-label">Visit Type</label>					
	...
	<?php echo form_open(base_url(), array("class"=>"form-horizontal", "id"=>"VMFormSection", "enctype"=>"multipart/form-data", "method"=>"post"));?>
		...
		<div class="row" id="MERAppSection" style="display: none;" >
			...
			<div class="col-md-8">
				<div class="row">
					<div class="col-md-3">
						<label for="prefered_lang" class="control-label">Prefer Language</label>
...
$(document).ready(function() {	
	...
	$("#SearchCaseData").click(function(){
		...
		if($appNumber !="" && $appNumber !="0" && $ic_id !="" ){
			$(".se-pre-con").show();	
			$.ajax({
				url:"<?php echo base_url();?>login/login/getCaseList",
				data: {'application_no':$appNumber,'ic_id':$ic_id,'csrf_test_name':csrf_token},
				dataType: 'json',
				method:'post',
			...
			alert("Please Enter Insurance Company Name & Application No");
		}
	});
...
function getNearestDC() {
	...
	if((lat =='' || lat =='0') && (pincode =="" || pincode=="0")) {			
		$("#displayError").html("<font color='red'>* Enter valid Pincode or Area/Locality to get neasrest Diagnostic Center</font>");
		return false;		
	} 
	else if(flag !='' && flag !='0'){
			$(".se-pre-con").show();
...
var today = new Date();
	...
	function getTimeSlot(equality){

		$.ajax({
			url: "<?php echo URL;?>check_status/Check_status/getTimeSlot",
			data: {'equality':equality,'csrf_test_name':csrf_token},
			type: "POST",
			dataType :'json',
			success: function(res){	
				if(res['status']==200){
	...
	$(function() {
	  ...
	  var mDate = new Date(currentYear, currentMonth, currentDate + nextday);  
	  var maxDate = new Date(currentYear, currentMonth, currentDate + 30);  
	  var mDateD = mDate.getDate();
	  var maxDateD = maxDate.getDate();
	  //console.log(mDate);

	  $('#mer_app_date').datetimepicker({
	    minDate: mDate,
	    maxDate:maxDate,
	    format: 'DD/MM/YYYY',
	  });

	  $('#mer_app_date').on('dp.change', function(){ 
	  	$ddd = $(this).val();
	  	$newdate = $ddd.split('/');
	...
	$('#ic_name').keyup(delayKeyup(function (e) {
		var icName = $(this).val();
		$("#ic_id").val("");
		if(icName.length >3){
			$("#suggesstion-box").show();
  			$("#suggesstion-box").html('<span style="color:red">Please Wait...</span>');
			$.ajax({
				url:"<?php echo base_url();?>getICNameList",
				data: {icName:icName,'csrf_test_name':csrf_token},
				dataType: 'html',
				method:'post',		
				success: function(res){							
			  		//console.log(res);
			  		$("#suggesstion-box").show();
			  		$('#suggesstion-box').find('li').last().click();
	...
	$(document).on('click','.icList', function(){
		var ic_id = $(this).attr('data-ic_id');
		var ic_name = $(this).text();
		$("#ic_id").val(ic_id);
		$("#ic_name").val(ic_name);
		$("#suggesstion-box").hide();	
		if(ic_id ==2){
			$("#data_type").html('<option value="">Select Search Option</option><option value="case_id">Case ID/TA Code</option><option value="application_id" >Member Id/Pivotal Application No</option><option value="customer" >Customer Email id/mobile no.</option>');
		} else{
			$("#data_type").html('<option value="">Select Search Option</option><option value="case_id">Case ID/TA Code</option><option value="application_id" >Application</option><option value="customer" >Customer Email id/mobile no.</option>');
		}
